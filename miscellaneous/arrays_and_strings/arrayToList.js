...
nested set of objects, with the first object holding a reference to the second, the second to the third, and so on.
...
referring to the variable defined earlier), they are both independent lists, but they share 
the structure that makes up their last three elements. In addition, the original list is also
...
for (var i =1; i < count.length; i++){
  var item=count[i];
  box.push(parseInt(item.slice(0,2)))
}
return box
}
